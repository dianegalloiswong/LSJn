...
let list_assoc l f = try Some (assoc f l)  with Not_found -> None;;
...
let new_linker lnk i prg = 
  let lc = map (recfun_length lnk) prg in
...
let compile prg exp =
  ...
  in  HALT::flatten (map (fun (_,x,_) -> x) (code_exp::code_fun));;
