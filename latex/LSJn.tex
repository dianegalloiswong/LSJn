...
\begin{df*}
Un \textbf{\emph{séquent}} de \LJ\ est la donnée d'un multiensemble de formules $\G$ (les ``hypothèses'') et d'une formule $D$ (la ``conclusion'') ; on écrit $\G \To D$.
\end{df*}



Les \textbf{règles} du calcul \LJ\ sont données dans la figure~\ref{fig:reglesLJ}.  Pour une règle \regle, $\mathcal R$ est le nom de la règle, $prem_{1}$, ... , $prem_{p}$ sont les \textbf{prémisses}, et $concl$ la \textbf{conclusion}. Les prémisses et la conclusion sont des séquents où $A$, $B$, $D$ sont des formules quelconques et $\G$ un multiensemble de formules quelconques. La distinction entre règles \emph{logiques} et \emph{structurelles} sera expliquée plus loin.
