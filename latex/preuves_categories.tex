...
\noindent
qui sont équivalentes selon le p.é.c.\,. Cela signifie précisément que $([p_{1}]\circ[p_{2}])\circ[p_{3}] = [p_{1}]\circ([p_{2}]\circ[p_{3}])$. L'identité pour une formule $A$ est la dénotation de la preuve 
{\centerAlignProof
	\AXC	{}
	\RightLabel{$(id)$}
	\UIC{$A\vdash A$}
...
Le nom de \emph{catégorie monoïdale} vient du fait que l'ensemble des objets de la catégorie (s'il s'agit bien d'un ensemble) muni de $\otimes$ est un monoïde.
\vspace{.5em}
