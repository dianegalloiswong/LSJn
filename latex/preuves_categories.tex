...
Voici un calcul de séquents pour la logique linéaire. Les séquents sont les mêmes que ceux de \LK. Les connecteurs, différents, sont donnés dans la figure~\ref{?}. Les règles sont données dans la figure~\ref{?}. Des explications sur ces connecteurs et ces règles sont proposées dans la sous-section~\ref{?}, puis une interprétation en terme de ``ressources'' dans la sous-section~\ref{?}. Enfin, on présente la logique linéaire intuitionniste (ILL).%, obtenue à partir de la logique linéaire de la même manière que la logique intuitionniste est obtenue à partir de la logique classique.
