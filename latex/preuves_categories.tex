...
Intéressons-nous à la compatibilité de $\otimes$ avec $\circ$. Soit des morphismes de  $f_{1} : A_{1}\to B_{1}$, $f_{2} : A_{2}\to B_{2}$, $g_{1} : B_{1}\to C_{1}$, $g_{2} : B_{2}\to C_{2}$ (on confond un objet avec sa dénotation pour simplifier l'écriture). Pour chaque morphisme $f$, soit $p(f)$ une preuve de dénotation $f$. Les deux preuves suivantes sont équivalentes selon le p.é.c., ce qui signifie que $(g_{1}\otimes g_{2})\circ(f_{1}\otimes f_{2}) = (g_{1}\circ f_{1})\otimes(g_{2}\circ f_{2})$.
\vspace{2mm}
